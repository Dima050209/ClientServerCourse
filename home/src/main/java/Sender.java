...
public class Sender {
    ...
    public void sendMessage(byte[] command){
        System.out.println(decrypt(command));
    }

    public Command decrypt(byte[] packet) {
        try {
            ...
            System.arraycopy(packet, 16, rawMessage, 0, messageLength);
            Cipher cipher = Cipher.getInstance(algorithm);
            cipher.init(Cipher.DECRYPT_MODE, key);

            byte[] messageBuffer = cipher.doFinal(rawMessage);
            ...
            byte[] header = ByteBuffer.allocate(1 + 1 + 8 + 4)
                    ...
                    .array();
            short packetCrc = wrap.getShort(14);
            if(packetCrc != crc16(header)){
                throw new RuntimeException();
            }

            return objectMapper.readValue(messageBuffer, Command.class);
